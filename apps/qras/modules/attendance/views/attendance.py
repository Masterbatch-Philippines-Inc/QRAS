...
#     # OvertimeRequest, UndertimeRequest, HalfdayRequest, 
#     EmployeeRestDay, MissingLog,
# )
# from app.attendance.helpers import (
#     format_hours_to_text, resolve_display_status,
#     check_duplicate_log, record_attendance, get_absent_employees,
#     # detect_and_sync_missing_logs,
# )
...
#     def get(self, request):
#         # # Auto-detect for yesterday every time the page loads
#         # yesterday = date_type.today() - timedelta(days=1)
#         # detect_and_sync_missing_logs(yesterday)
...
#         #     time_in_log = AttendanceLog.objects.filter(
#         #         employee=ml.employee, date=ml.date, is_time_in=True, is_replaced=False
#         #     ).order_by('timestamp').first()
#         #     time_out_log = AttendanceLog.objects.filter(
#         #         employee=ml.employee, date=ml.date, is_time_out=True, is_replaced=False
#         #     ).order_by('timestamp').first()
#         #     ml.time_in  = timezone.localtime(time_in_log.timestamp)  if time_in_log  else None
#         #     ml.time_out = timezone.localtime(time_out_log.timestamp) if time_out_log else None
...
#         try:
#             data    = json.loads(request.body)
#             action  = data.get('action')   # 'resolve' or 'dismiss'
#             log_id  = data.get('id')
...
#             action      = data.get('log_action')   # 'in' or 'out'
#             date_str    = data.get('date')
